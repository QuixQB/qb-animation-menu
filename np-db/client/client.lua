...
function setNuiActive(booleanOrNil)
  ...
  if boolean ~= isNuiActive then
    if boolean then TriggerServerEvent('np-db:request-data') end
    isNuiActive = boolean
...
CreateThread(function()
  if isNuiActive then TriggerServerEvent('np-db:request-data') end
  Wait(300000)
end)
...
RegisterNetEvent('np-db:update-resource-data')
AddEventHandler('np-db:update-resource-data', function (resourceData)
...
RegisterNetEvent('np-db:update-time-data')
AddEventHandler('np-db:update-time-data', function (timeData)
  local timeArray = {}
...
RegisterNetEvent('np-db:update-slow-queries')
AddEventHandler('np-db:update-slow-queries', function(slowQueryData)
...
RegisterNetEvent('np-db:update-status')
AddEventHandler('np-db:update-status', function(statusData)
  SendNUIMessage({
    type = 'onStatusData',
    status = statusData,
  });
...
RegisterNetEvent('np-db:update-variables')
AddEventHandler('np-db:update-variables', function(variableData)
  SendNUIMessage({
    type = 'onVariableData',
...
RegisterNUICallback('request-server-status', function()
  TriggerServerEvent('np-db:request-server-status')
end)

RegisterNetEvent('np-db:get-table-schema')
AddEventHandler('np-db:get-table-schema', function(tableName, schema)
  SendNUIMessage({
    type = 'onTableSchema',
    tableName = tableName,
    schema = schema,
...
RegisterNUICallback('request-table-schema', function(tableName)
  TriggerServerEvent('np-db:request-table-schema', tableName)
end)
